...
def get_ocf_ev(symbols):
    ev = None
    ocf_ev_dict = dict()    
    for symbol in symbols:
        try:
            keystat = keystatfunc(symbol)
            ev_value = keystat[0]
            ocf_value = keystat[28]
            if 'M' in ev_value:
                ev = float(ev_value[:len(ev_value)-2])
            elif 'B' in ev_value:
                ev = float(ev_value[:len(ev_value)-2]) * pow(10, 3)
            if 'M' in ocf_value:
                ocf = float(ocf_value[:len(ocf_value)-2])
            elif 'B' in ocf_value:
                ocf = float(ocf_value[:len(ocf_value)-2]) * pow(10, 3)
            ocf_ev_dict[symbol] = ocf / ev
        except:
            ocf_ev_dict[symbol] = np.nan
    ocf_ev_df = pd.DataFrame(ocf_ev_dict.items(), columns=['ticker', 'ocf_ev'])
    ocf_ev_df.set_index('ticker', inplace=True)    
    return ocf_ev_df
...
def get_cfo(symbols):
    ...
    cfo_dict = dict()    
    for symbol in symbols:
        value = keystatfunc(symbol)[28]
        if 'M' in value:
            cfo = float(value[:len(value)-2])
        elif 'B' in value:
            cfo = float(value[:len(value)-2]) * pow(10, 3)
        cfo_dict[symbol] = cfo
    cfo_df = pd.DataFrame(cfo_dict.items(), columns=['tic', 'trail_cfo'])
    cfo_df.set_index('tic', inplace=True)
    return cfo_df
